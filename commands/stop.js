...
async function execute(interaction) {
	...
	if (!bot.isPinging) {
		interaction.reply({
			content: 'What ever do you mean "stop"?',
			ephemeral: true,
		});
		return;
	} else if (
		messageAuthor != spam.spamVictim.id ||
		messageAuthor != bot.ownerID ||
		messageAuthor != spam.spamStarter
	) {
		interaction.reply({
			content: "This does not concern you, go along now",
			ephemeral: true,
		});
		return;
	}
